...
QList<ScColorProfileInfo> ScLcms2ColorMgmtEngineImpl::getAvailableProfileInfo(const QString& directory, bool recursive)
{
	...
	for (uint dc = 0; dc < d.count(); ++dc)
	{
		...
		cmsHPROFILE hIn = cmsOpenProfileFromFile(profilePath.data(), "r");
		if (hIn == nullptr)
			continue;
...
ScColorProfile ScLcms2ColorMgmtEngineImpl::openProfileFromFile(ScColorMgmtEngine& engine, const QString& filePath)
{
	...
	if (file.open(QFile::ReadOnly))
	{
		...
		if (!data.isEmpty())
		{
			...
			if (lcmsProf)
			{
				auto* profData = new ScLcms2ColorProfileImpl(engine, lcmsProf);
                                 ...
                                 eRenderIntent renderIntent, long transformFlags)
{
	...
	const auto* lcmsInputProf  = dynamic_cast<const ScLcms2ColorProfileImpl*>(inputProfile.data());
	const auto* lcmsOutputProf = dynamic_cast<const ScLcms2ColorProfileImpl*>(outputProfile.data());
	...
	if (transform.isNull())
	{
		...
		if (hTransform)
		{
			auto* newTrans = new ScLcms2ColorTransformImpl(engine, hTransform);
                                             ...
                                             eRenderIntent proofingIntent, long transformFlags)
{
	...
	const auto* lcmsInputProf    = dynamic_cast<const ScLcms2ColorProfileImpl*>(inputProfile.data());
	const auto* lcmsOutputProf   = dynamic_cast<const ScLcms2ColorProfileImpl*>(outputProfile.data());
	const auto* lcmsProofingProf = dynamic_cast<const ScLcms2ColorProfileImpl*>(proofProfile.data());
...
{
	static_assert(sizeof(wchar_t) == 2 || sizeof(wchar_t) == 4, "wchar_t size is unexpected");
	...
	if (sizeof(wchar_t) == 4)
	{
		...
		std::unique_ptr<ushort[]> utf16Data(new (std::nothrow) ushort[(size_t) size + 1]);
		if (utf16Data)
		{
			for (int i = 0; i < size; ++i)
				utf16Data[i] = static_cast<ushort>(wstr[i]);
			utf16Data[size] = 0;
