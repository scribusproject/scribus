...
void drawCircularHandle(QPainter *painter, QPointF center, qreal width,
                        QBrush background, bool isEnabled) {
  ...
  // Draw white ring
  painter->setPen(QPen(
      ScQApp->palette().color(isEnabled ? QPalette::Active : QPalette::Disabled,
                              QPalette::Window),
      2));
...
void drawPointerHandle(QPainter *painter, QPointF pointer, qreal width,
                       QColor background, bool isEnabled, bool isEmpty) {
  ...
  if (isEmpty)
    painter->drawPixmap(
        rect,
        renderEmptyPattern(rect.size(), painter->device()->devicePixelRatio()));
  else
    painter->drawPixmap(rect, renderColor(rect.size(),
...
void drawSelectionHandle(QPainter *painter, QPointF point, QPen pen,
                         qreal scaleFactor, bool isActive) {
  QRectF handleRect(0, 0, 10 / scaleFactor, 10 / scaleFactor);
...
void drawNodeHandle(QPainter *painter, QPointF point, QPen pen,
                    qreal scaleFactor, bool isActive) {
  drawSelectionHandle(painter, point, pen, scaleFactor, isActive);
}

void drawNodeControl(QPainter *painter, QPointF point, QPen pen,
                     qreal scaleFactor, bool isActive) {
...
void drawWeldMarker(QPainter *painter, QPointF point, QColor color,
                    qreal scaleFactor) {
  bool aa = painter->testRenderHint(QPainter::Antialiasing);
...
void renderCheckerPattern(QPainter *painter, QRectF rect) {
  QImage pattern(rect.width(), rect.height(),
  ...
  for (int y = 0; y < cellSizeY; y++)
    for (int x = y % 2; x < cellSizeX; x += 2)
      p.fillRect(QRectF(x * cellSize, y * cellSize, cellSize, cellSize),
                 QBrush(Qt::lightGray));
...
QPixmap renderColor(QSize size, double scale, QColor color, QColor colorShade,
                    double alpha) {
...
QPixmap renderGradientLinear(QSize size, double scale,
  ...
  p->setGradient(VGradient::linear, FPoint(0, h / 2), FPoint(w, h / 2),
                 FPoint(0, 0), 1, 0);
  p->drawRect(0, 0, w, h);
  p->end();
  delete p;
...
QPixmap renderGradientRadial(QSize size, double scale,
                             const VGradient &gradient) {
  int w = size.width();
  int h = size.height();
  qreal wHalf = w / 2;
  ...
  p->setFillMode(ScPainter::Gradient);
  p->fill_gradient = gradient;
  p->setGradient(VGradient::radial, FPoint(wHalf, hHalf), FPoint(w, hHalf),
                 FPoint(wHalf, hHalf), 1, 0);
  p->drawRect(0, 0, w, h);
  p->end();
  delete p;
...
QPixmap renderGradientConical(QSize size, double scale,
                              const VGradient &gradient) {
  int w = size.width();
  int h = size.height();
  qreal wHalf = w / 2;
  ...
  p.setPen(Qt::NoPen);
  p.setBrush(grad);
  p.translate(0, h); // we have to flip vertically, because QConicalGradient is
...
QPixmap renderGradient4Colors(QSize size, double scale, QColor col1,
                              QColor col2, QColor col3, QColor col4) {
  int w = size.width();
  int h = size.height();
  ...
  p->set4ColorGeometry(FPoint(0, 0), FPoint(w, 0), FPoint(w, h), FPoint(0, h),
                       FPoint(0, 0), FPoint(w, 0), FPoint(w, h), FPoint(0, h));
...
QPixmap renderGradientDiamond(QSize size, double scale,
                              const VGradient &gradient) {
  int w = size.width();
  int h = size.height();
  ...
  p->setFillMode(ScPainter::Gradient);
  p->fill_gradient = gradient;
  p->setDiamondGeometry(FPoint(0, 0), FPoint(w, 0), FPoint(w, h), FPoint(0, h),
                        FPoint(0, 0), FPoint(w, 0), FPoint(w, h), FPoint(0, h),
                        FPoint(w / 2, h / 2));
  p->drawRect(0, 0, w, h);
  p->end();
...
QPixmap renderHatch(QSize size, double scale, int type, double distance,
                    double angle, bool hasBackground, QColor backgroundColor,
  ...
  p->setHatchParameters(type, distance, angle, hasBackground, backgroundColor,
                        foregroundColor, w, h);
  p->drawRect(0, 0, w, h);
  p->end();
  delete p;
...
QPixmap combinePixmaps(const QPixmap &background, const QPixmap &foreground,
                       double scale, bool tintForeground, bool isDarkColor) {
  int w = qMax(background.deviceIndependentSize().width(),
               foreground.deviceIndependentSize().width());
  int h = qMax(background.deviceIndependentSize().height(),
               foreground.deviceIndependentSize().height());
  ...
  QRect rB(0, 0, background.deviceIndependentSize().width(),
           background.deviceIndependentSize().height());
  rB.moveCenter(frame.center());

  QRect rF(0, 0, foreground.deviceIndependentSize().width(),
           foreground.deviceIndependentSize().height());
  ...
  p.setCompositionMode(tintForeground ? QPainter::CompositionMode_SourceIn
...
VColorStop computeInBetweenStop(const VColorStop *last,
                                const VColorStop *actual, double t) {
  double dist = actual->rampPoint - last->rampPoint;
  double perc = (t - last->rampPoint) / dist;
  double Rn =
      ((actual->color.redF() - last->color.redF()) * perc) + last->color.redF();
  double Gn = ((actual->color.greenF() - last->color.greenF()) * perc) +
              last->color.greenF();
  double Bn = ((actual->color.blueF() - last->color.blueF()) * perc) +
              last->color.blueF();
  QColor color;
...
QColor disabledColor(QColor color) {
  QColor paleColor =
      blendColor(.5, color, ScQApp->palette().color(QPalette::Window));
...
bool isDarkColor(QColor color) {
  ...
  return color.alphaF() > 0 &&
         lum <= 0.65; // It is a dark color if the luminance is below 65%
}
