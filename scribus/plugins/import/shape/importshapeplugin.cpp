...
{
	auto* plug = new ImportShapePlugin();
	Q_CHECK_PTR(plug);
	return plug;
}
...
bool ImportShapePlugin::importFile(QString fileName, int flags)
{
	if (!checkFlags(flags))
		return false;
	if (fileName.isEmpty())
	...
	auto pPlug = std::make_unique<ShapePlug>(m_Doc, flags);
	Q_CHECK_PTR(pPlug);
...
QImage ImportShapePlugin::readThumbnail(const QString& fileName)
{
	...
	UndoManager::instance()->setUndoEnabled(false);
	m_Doc = nullptr;
	auto pPlug = std::make_unique<ShapePlug>(m_Doc, lfCreateThumbnail);
	Q_CHECK_PTR(pPlug);
