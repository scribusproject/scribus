...
bool ImportQxpPlugin::importFile(QString fileName, int flags)
{
	if (!checkFlags(flags))
		return false;
	if (fileName.isEmpty())
	...
	auto pPlug = std::make_unique<QxpPlug>(m_Doc, flags);
	Q_CHECK_PTR(pPlug);
...
QImage ImportQxpPlugin::readThumbnail(const QString& fileName)
{
	...
	UndoManager::instance()->setUndoEnabled(false);
	m_Doc = nullptr;
	auto pPlug = std::make_unique<QxpPlug>(m_Doc, lfCreateThumbnail);
	Q_CHECK_PTR(pPlug);
