...
bool ImportSmlPlugin::importFile(QString fileName, int flags)
{
	if (!checkFlags(flags))
		return false;
	if (fileName.isEmpty())
	...
	auto pPlug = std::make_shared<SmlPlug>(m_Doc, flags);
	Q_CHECK_PTR(pPlug);
...
QImage ImportSmlPlugin::readThumbnail(const QString& fileName)
{
	...
	UndoManager::instance()->setUndoEnabled(false);
	m_Doc = nullptr;
	auto pPlug = std::make_shared<SmlPlug>(m_Doc, lfCreateThumbnail);
	Q_CHECK_PTR(pPlug);
