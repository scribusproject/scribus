...
bool ImportXarPlugin::importFile(QString fileName, int flags)
{
	if (!checkFlags(flags))
		return false;
	if (fileName.isEmpty())
	...
	auto pPlug = std::make_unique<XarPlug>(m_Doc, flags);
	Q_CHECK_PTR(pPlug);
...
QImage ImportXarPlugin::readThumbnail(const QString& fileName)
{
	...
	UndoManager::instance()->setUndoEnabled(false);
	m_Doc = ScCore->primaryMainWindow()->doc;
	auto pPlug = std::make_unique<XarPlug>(m_Doc, lfCreateThumbnail);
	Q_CHECK_PTR(pPlug);
