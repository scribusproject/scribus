...
bool ImportCvgPlugin::importFile(QString fileName, int flags)
{
	if (!checkFlags(flags))
		return false;
	if (fileName.isEmpty())
	...
	auto dia = std::make_shared<CvgPlug>(m_Doc, flags);
	Q_CHECK_PTR(dia);
...
QImage ImportCvgPlugin::readThumbnail(const QString& fileName)
{
	...
	UndoManager::instance()->setUndoEnabled(false);
	m_Doc = nullptr;
	auto dia = std::make_shared<CvgPlug>(m_Doc, lfCreateThumbnail);
	Q_CHECK_PTR(dia);
