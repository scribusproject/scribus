...
bool ImportPmPlugin::importFile(QString fileName, int flags)
{
	if (!checkFlags(flags))
		return false;
	if (fileName.isEmpty())
	...
	auto dia = std::make_shared<PmPlug>(m_Doc, flags);
	Q_CHECK_PTR(dia);
...
QImage ImportPmPlugin::readThumbnail(const QString& fileName)
{
	...
	UndoManager::instance()->setUndoEnabled(false);
	m_Doc = nullptr;
	auto dia = std::make_shared<PmPlug>(m_Doc, lfCreateThumbnail);
	Q_CHECK_PTR(dia);
