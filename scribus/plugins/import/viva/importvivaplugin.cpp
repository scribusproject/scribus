...
{
	auto* plug = new ImportVivaPlugin();
	Q_CHECK_PTR(plug);
	return plug;
}
...
bool ImportVivaPlugin::importFile(QString fileName, int flags)
{
	if (!checkFlags(flags))
		return false;
	if (fileName.isEmpty())
	...
	auto pPlug = std::make_unique<VivaPlug>(m_Doc, flags);
	Q_CHECK_PTR(pPlug);
...
QImage ImportVivaPlugin::readThumbnail(const QString& fileName)
{
	...
	UndoManager::instance()->setUndoEnabled(false);
	m_Doc = nullptr;
	auto pPlug = std::make_unique<VivaPlug>(m_Doc, lfCreateThumbnail);
	Q_CHECK_PTR(pPlug);
