...
void nftdialog::setTNails()
{
	if (categoryList->currentItem() == 0)
	{
		...
		{
			QListWidgetItem* tmpQIVI = new QListWidgetItem(QPixmap(iconItems[i]->first->tnail),
			                                           iconItems[i]->first->name, tnailGrid);
// 			tnailGrid->addItem(tmpQIVI);
...
void nftdialog::setInfo() 
{
	...
	/*QListWidgetItem* qvi = */new QListWidgetItem(tmplImg, currentDocumentTemplate->name, imageView);
