...
class PLUGIN_API PathAlongPathPlugin : public ScActionPlugin
{
	...
	private:
		...
		PageItem *m_pathItem;
		FPointArray m_originalPath;
		FPointArray m_effectPath;
		double m_originalRot;
		double m_originalXPos;
		double m_originalYPos;
		ScribusDoc* m_doc;
		bool m_firstUpdate;
		Geom::Piecewise<Geom::D2<Geom::SBasis> > m_uskeleton;
		Geom::Piecewise<Geom::D2<Geom::SBasis> > m_n;
		double m_scaling;
		int m_nbCopies;
		double m_pattWidth;
		double m_offsetX;
		...
		double m_gapval;
		int m_rotate;
		Geom::Interval m_pattBnds;
		Geom::Interval m_pattBndsY;
		int m_selOffs;
		int m_selCount;
		QList<FPointArray> m_originalPathG;
		QList<double> m_originalRotG;
		QList<double> m_originalXPosG;
		QList<double> m_originalYPosG;
		QList<double> m_originalXPosGi;
		QList<double> m_originalYPosGi;
		QList<double> m_originalWidthG;
		QList<double> m_originalHeightG;
		QList<double> m_originalWidth;
		QList<double> m_originalHeight;
		QList<PageItem *> m_patternItemG;
	private slots:
