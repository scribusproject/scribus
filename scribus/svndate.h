...
inline QDateTime getSvnDateTime()
{
    ...
    // Adjust the time based on time zone information. We discard the minutes
    // from the timezone since we just don't care that much. We need to convert
    // to UTC because QDateTime's fromString can't understand the timezone
    // natively, and there's no guarantee that the local time where this copy
    // of svn was checked out is the same as the local time the program is
    // being run in.
