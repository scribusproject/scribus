...
namespace RtfReader
{
	...
	{
	/*	QString fontName = m_textCharStyle.top().font().replacementName();
		QString family = m_textCharStyle.top().font().family();
		...
		if (value != 0)
		{
			...
			if (slist.count() > 0)
			{
				int reInd = slist.indexOf("Bold");
				if (reInd < 0)
					return;
				else
					fontName = m_textCharStyle.top().font().family() + " " + slist[reInd];
				return;
			}
		}*/
	//	qDebug() << "setFontBold";
