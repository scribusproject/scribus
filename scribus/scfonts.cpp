...
void SCFonts::AddScalableFonts(const QString &path)
{
	...
	if ((d.exists()) && (d.count() != 0))
	{
		for (uint dc = 0; dc < d.count(); ++dc)
		{
			...
			{
				if (!AddScalableFont(pathfile, library))
					qDebug(QObject::tr("Font %1 is broken, discarding it")
							.arg(pathfile));
			}
		}
	}
...
// Load a single font into the library from the passed filename. Returns true
// on success.
bool SCFonts::AddScalableFont(QString filename, FT_Library &library)
{
	FT_Face face = NULL;
...
void SCFonts::AddFontconfigFonts()
{
	...
	FcPattern* pat = FcPatternBuild(NULL, FC_SCALABLE,
			FcTypeBool, true, NULL);
	// The ObjectSet tells FontConfig what information about each match to
	// return.  We currently just need FC_FILE, but other info like font
	// family and style is availible - see "man fontconfig".
	...
	for (i = 0; i < fs->nfont; i++) {
		...
		{
			if (!AddScalableFont(QString((char*)file), library))
				qDebug(QObject::tr("Font %1 (found using fontconfig) is broken, discarding it").arg(QString((char*)file)).local8Bit().data());
		}
		else
			qDebug(QObject::tr("Failed to load a font - freetype couldn't find the font file").local8Bit().data());
	}
	FT_Done_FreeType( library );
#endif
}
