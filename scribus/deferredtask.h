...
// DeferredTask is an abstraction of a generally long-running operation
...
public:
	// If you override the destructor, call DeferredTask::cleanup() at the
	// end of your dtor.
	virtual ~DeferredTask();

	// Returns true iff processing has finished. Note that this will return false
	// both while processing, and before start() has been called.
	bool isFinished() const;

	// Return a human-readable version of the last error message. Unless
...
public slots:
	...
	// Starts the timer and begins processing. If you override this you must
	// call DeferredTask::start() at the end of your start() implementation.
	// If something goes wrong, emit aborted();
	virtual void start();

	// Run continuously without returning control to the event loop, until
	...
	// Abort the search and emit aborted(true). This is treated as an abort by
...
signals:
	// Emitted if the processing work was aborted before completion. The signal
	...
	// Emitted when processing is complete, from DeferredTask::done(). You can
...
protected:
	// You must provide a new, public, constructor. It should set up the class
	...
	// Indicate that processing has finished. You must call this before
...
private:
	// Status code indicating whether we're running, not yet started, failed,
	// etc.
	int m_status;

	// The timer used to control next() calls.
	QTimer* m_timer;
};
