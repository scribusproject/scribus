	...
	else if ((ext == "eps") || (ext == "ps"))
	{
		...
		if (found)
		{
			...
			if (retg == 0)
			{
				...
				if (ext == "eps")
				{
					if ((qRound(b - x) < image.width()) || (qRound(h - y) < image.height()))
					{
						int yc = qRound(image.height() - h);
						*this = static_cast<ScImage>(image.copy(qRound(x), yc, qRound(b - x), qRound(h - y)));
					}
					else
		...
		case RGBProof: // RGB Proof
			{
				if (SoftProofing && CMSuse)
				{
					xform = scCmsCreateProofingTransform(inputProf, inputProfFormat,
					                       ...
					                       IntentPrinter, INTENT_RELATIVE_COLORIMETRIC, cmsFlags | cmsProofFlags);
				}
				else
