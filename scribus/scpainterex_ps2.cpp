...
const QWMatrix ScPainterEx_Ps2::worldMatrix()
{
	return m_matrix;
}

void ScPainterEx_Ps2::setWorldMatrix( const QWMatrix &mat )
{
	m_matrix = mat;
}
...
void ScPainterEx_Ps2::setPattern( ScPattern* pattern, QWMatrix& patternTransform )
{
	m_pattern = pattern;
...
void ScPainterEx_Ps2::setDash(const QList<double>& array, double ofs)
{
	m_array = array;
	m_offset = ofs;
}
...
void ScPainterEx_Ps2::setupTextPolygon(FPointArray *points)
{
	bool nPath = true;
	FPoint np, np1, np2, np3;
	if (points->size() > 3)
	{
		...
		for (uint poi=0; poi<points->size()-3; poi += 4)
		{
			...
			{
				np = points->point(poi);
				// TODO closefigure / startfigure needed?
				moveTo( np.x(), np.y() );
				nPath = false;
			}
...
void ScPainterEx_Ps2::getPathBoundingBox( FPointArray* points, QRect& r )
{
	...
	r.setCoords( left, bottom, right, top );
}
