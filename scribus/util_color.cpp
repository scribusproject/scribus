...
QImage ProofImage(const QImage *Image, const ScribusDoc* doc)
{
...
QPixmap * getFancyPixmap(const ScColor& col, const ScribusDoc* doc)
{
	...
	if (!iconsInitialized)
	{
		...
		alertIcon = im.loadPixmap("alert-warning");
		cmykIcon = im.loadPixmap("color-cmyk");
		rgbIcon = im.loadPixmap("color-rgb");
		labIcon = im.loadPixmap("color-lab");
		spotIcon = im.loadPixmap("color-spot");
		regIcon = im.loadPixmap("color-registration");
...
bool importColorsFromFile(const QString& fileName, ColorList &EditColors, QHash<QString, VGradient> *dialogGradients, bool merge)
{
	if (fileName.isEmpty())
		return false;
	qsizetype oldCount = EditColors.count();
