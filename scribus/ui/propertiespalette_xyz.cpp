...
PropertiesPalette_XYZ::PropertiesPalette_XYZ( QWidget* parent) : QWidget(parent)
{
    ...
    connect(basePointWidget, SIGNAL(buttonClicked(BasePointWidget::AnchorPoint)), this, SLOT(handleBasePoint(BasePointWidget::AnchorPoint)));
...
void PropertiesPalette_XYZ::handleSelectionChanged()
{
	...
	if (m_doc->m_Selection->count() > 1)
	{
        ...
        BasePointWidget::AnchorPoint bp = basePointWidget->selectedAnchor();
        if (bp == BasePointWidget::AnchorPoint::TopLeft)
			m_ScMW->view->RCenter = FPoint(gx, gy);
        else if (bp == BasePointWidget::AnchorPoint::TopRight)
			m_ScMW->view->RCenter = FPoint(gx + gw, gy);
        else if (bp == BasePointWidget::AnchorPoint::Center)
			m_ScMW->view->RCenter = FPoint(gx + gw / 2.0, gy + gh / 2.0);
        else if (bp == BasePointWidget::AnchorPoint::BottomLeft)
			m_ScMW->view->RCenter = FPoint(gx, gy + gh);
        else if (bp == BasePointWidget::AnchorPoint::BottomRight)
...
void PropertiesPalette_XYZ::showXY(double x, double y)
{
    ...
    if (bp == BasePointWidget::AnchorPoint::TopLeft || useLineMode)
		n = FPoint(0.0, 0.0);
    else if (bp == BasePointWidget::AnchorPoint::TopRight)
		n = FPoint(b, 0.0);
    else if (bp == BasePointWidget::AnchorPoint::Center)
...
void PropertiesPalette_XYZ::handleNewX()
{
	...
	if (m_doc->m_Selection->isMultipleSelection())
	{
		...
		m_doc->m_Selection->getGroupRect(&gx, &gy, &gw, &gh);
        BasePointWidget::AnchorPoint bp = basePointWidget->selectedAnchor();
        if ((bp == BasePointWidget::AnchorPoint::TopLeft) || (bp == BasePointWidget::AnchorPoint::BottomLeft))
			base = gx;
        else if (bp == BasePointWidget::AnchorPoint::Center)
			base = gx + gw / 2.0;
        else if ((bp == BasePointWidget::AnchorPoint::TopRight) || (bp == BasePointWidget::AnchorPoint::BottomRight))
			base = gx + gw;
		if (!m_userActionOn)
...
void PropertiesPalette_XYZ::handleNewY()
{
	...
	if (m_doc->m_Selection->isMultipleSelection())
	{
		...
		m_doc->m_Selection->getGroupRect(&gx, &gy, &gw, &gh);
        BasePointWidget::AnchorPoint bp = basePointWidget->selectedAnchor();
        if ((bp == BasePointWidget::AnchorPoint::TopLeft) || (bp == BasePointWidget::AnchorPoint::TopRight))
			base = gy;
        else if (bp == BasePointWidget::AnchorPoint::Center)
			base = gy + gh / 2.0;
        else if ((bp == BasePointWidget::AnchorPoint::BottomLeft) || (bp == BasePointWidget::AnchorPoint::BottomRight))
			base = gy + gh;
		if (!m_userActionOn)
			m_ScMW->view->startGroupTransaction();
...
void PropertiesPalette_XYZ::handleBasePoint(BasePointWidget::AnchorPoint m)
{
	if (!m_ScMW || m_ScMW->scriptIsRunning())
		return;
