...
//			qDebug() << "UndoManager::cancelTransaction" << data << data->transactionObject->getUName() << data->transactionState->getName() << stackLevel;
			...
			//brutal for now:
			assert (stackLevel + 1 == signed(UM->m_transactions_.size()));
			if (stackLevel < signed(UM->m_transactions_.size()))
			{
				UM->m_transactions_.erase(UM->m_transactions_.begin() + stackLevel);
			}
			return true;
		default:
			// do nothing;
...
bool UndoTransaction::commit()
{
	...
	if (!UM->m_undoEnabled_)
	{
		cancel();
		return false;
	}
	
	UndoObject *tmpu = UM->m_transactions_.at(stackLevel)->transactionObject;
	TransactionState *tmps = UM->m_transactions_.at(stackLevel)->transactionState;
